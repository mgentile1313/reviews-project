...
from scripts.lib.config import LABEL_MODEL
...
# Reviews per cluster sent to Haiku.
REP_PER_CLUSTER = 5
...
# ---------------------------------------------------------------------------
# Stage 1 — surface up to 3 distinct concrete practices in a cluster
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Stage 2 — pick one candidate per cluster, distinct across the pass
# ---------------------------------------------------------------------------
...
def _format_review_for_prompt(review: dict, idx: int) -> str:
...
def _parse_haiku_json(raw: str) -> dict:
    """Extract the first JSON object from Haiku's response.

    Tolerant of:
      - Leading/trailing whitespace
      - Markdown code fences (``` or ```json)
      - Leading prose before the JSON object (skips to first '{')
      - Trailing prose after the JSON object (raw_decode stops at object end)

    The prompt asks Haiku to return JSON only, but if it slips in an explanatory
    paragraph after the object — as it sometimes does on generic clusters
    where the model wants to justify an empty result — we still parse cleanly
    instead of dying on `Extra data`.
...
def _haiku_json_call(client, system_prompt: str, user_msg: str, max_tokens: int) -> dict | None:
    ...
    for attempt in (1, 2):
        try:
            ...
            return _parse_haiku_json(res.content[0].text)
...
def stage1_candidates(
    ...
    user_msg = (
        ...
        + "\n\n".join(
            _format_review_for_prompt(r, i + 1) for i, r in enumerate(representatives)
        )
        + "\n\nReturn your JSON candidate list."
    )
    parsed = _haiku_json_call(client, STAGE1_SYSTEM_PROMPT, user_msg, max_tokens=1000)
    if parsed is None:
        return None
    ...
    # Defensive validation — each candidate must have label + quote
    cleaned = []
    for c in candidates:
...
def stage2_select(
    ...
    """Stage 2: pick one candidate or none. Always returns a dict
    (never None) — on Haiku failure, returns picked_index=null with a reason."""
    if not candidates:
    ...
    parsed = _haiku_json_call(client, STAGE2_SYSTEM_PROMPT, user_msg, max_tokens=300)
    if parsed is None:
    ...
    if picked is not None and not (isinstance(picked, int) and 0 <= picked < len(candidates)):
        log.warning("stage2 returned out-of-range picked_index=%s for %d candidates",
